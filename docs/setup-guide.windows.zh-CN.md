...
每个支持切换的前端服务卡片都有“后端连接”区域。
初始列表始终包含 `127.0.0.1（本机）`。
在输入框填写同事电脑的 IPv4 地址，例如 `192.168.1.44`，点击“添加 IP”后，该地址会保存在当前 Windows 用户的 `%LOCALAPPDATA%\KMCBServiceControl\runtime` 中。
从下拉列表选择地址并点击“应用并重启”。
如果前端正在运行，控制中心会用新地址重启该前端；如果前端已经停止，选择会在下次启动时生效。
页面会分别标明当前进程已生效的地址，以及尚未应用时的等待状态。
同事电脑必须允许你的电脑访问对应后端端口，且 Windows 防火墙、公司网络和后端监听地址不能阻断局域网访问。

后端地址通过服务注册表中的 `backendTarget.envTemplates` 注入，端口仍由项目配置决定。
添加 IP 不会修改业务仓库的 `.env` 文件，也不会把本机地址提交到 Git。
...
页面日志弹窗打开后会每秒增量读取新增内容，不需要重复点击刷新。
首次打开显示当前日志最近 `2 MiB`，窗口最多保留约 `8 MiB` 以避免浏览器长期占用过多内存。
需要检查更早的异常上下文时，点击弹窗中的“下载完整日志”，下载文件会按时间顺序合并上一次轮转日志和当前日志。
向上滚动查看旧内容时页面仍会接收新日志，但不会强制跳到底部；点击“跳到最新”可恢复到底部。
只有控制中心启动的服务会把输出写入这里，外部终端或 IntelliJ 启动的进程仍需在原启动位置查看日志。
